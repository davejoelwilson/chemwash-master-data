/**
 * update-airtable-fields.js
 * 
 * This script updates a specific job row in Airtable with contact information from Fergus.
 * Usage: node update-airtable-fields.js JOB_NUMBER
 * Example: node update-airtable-fields.js NW-21418
 */
 ...
 */
function extractContactInfo(jobData) {
  const contactInfo = {};
  ...
  if (jobData.customer && jobData.customer.billing_contact) {
    ...
    const addressParts = [
      billingContact.address_1,
      billingContact.address_2,
      billingContact.address_suburb,
      billingContact.address_city,
      billingContact.address_postcode
    ].filter(Boolean);
    
    contactInfo.billingAddress = addressParts.join(' ');
  }
...
/**
 * Find Airtable record for a specific job
 */
async function findAirtableRecord(jobId) {
  try {
    console.log(`Searching for Airtable record with Job ID: ${jobId}`);
    
    // First, get all records to find the right field name
    return new Promise((resolve, reject) => {
      // Let's first list all records to see what we have
      table.select({
        maxRecords: 10
        ...
        if (records && records.length > 0) {
          ...
          // Go through each record to find our job
          const foundRecord = records.find(record => {
            // Check all fields for the job ID
            for (const key in record.fields) {
              if (record.fields[key] === jobId) {
                console.log(`Found job ID in field: ${key}`);
                return true;
              }
            }
            return false;
          ...
          if (foundRecord) {
            console.log(`✅ Found Airtable record for ${jobId}`);
            resolve(foundRecord);
          } else {
            console.log(`❌ No record found for ${jobId}`);
            resolve(null);
          }
        } else {
...
/**
 * Update Airtable record with contact information
 */
async function updateAirtableRecord(recordId, contactInfo) {
  try {
    console.log(`Updating Airtable record ${recordId} with contact info...`);
    ...
    return new Promise((resolve, reject) => {
      ...
      table.update(recordId, cleanFields, (err, record) => {
        if (err) {
...
async function processJob(jobId) {
  try {
    ...
    const contactInfo = extractContactInfo(jobData);
    console.log('Extracted contact info:');
    console.log(JSON.stringify(contactInfo, null, 2));
    
    // Step 3: Find the record in Airtable
    const record = await findAirtableRecord(jobId);
    if (!record) {
      console.error(`❌ No Airtable record found for job ${jobId}`);
      return false;
    }
    
    // Step 4: Update the Airtable record
    const updatedRecord = await updateAirtableRecord(record.id, contactInfo);
    if (!updatedRecord) {
      console.error(`❌ Failed to update Airtable record for job ${jobId}`);
      return false;
    }
...
if (!jobId) {
  ...
  console.log('Usage: node update-airtable-fields.js NW-21255');
  process.exit(1);
}
