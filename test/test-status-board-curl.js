/**
 * Test script to access the Fergus Status Board Data endpoint
 * Using the exact curl command provided translated to axios
 */
const axios = require('axios');
const fs = require('fs').promises;
require('dotenv').config();

// Use exact cookies from the curl command
...
/**
 * Default headers matching exactly the curl command headers
 */
...
/**
 * Access the status_board/data endpoint
 */
async function fetchStatusBoardData() {
  try {
    console.log('Attempting to access status_board/data endpoint with curl cookies...');
    
    // Make a POST request as shown in the curl example
    ...
    await fs.writeFile('output/status-board-data-curl.json', JSON.stringify(response.data, null, 2));
    console.log('Saved response to output/status-board-data-curl.json');
    
    // Check for job statuses in the response
    if (response.data && response.data.job_statuses) {
      console.log('Available job statuses:');
      response.data.job_statuses.forEach(status => {
        console.log(`- ${status.id}: ${status.name}`);
      });
    }
    
    // Check for jobs with completed status
    const completedJobs = [];
    if (response.data && response.data.jobs) {
      console.log(`Found ${response.data.jobs.length} total jobs`);
      
      response.data.jobs.forEach(job => {
        if (job.status_name === 'Completed' || /complete|invoiced|paid/i.test(job.status_name)) {
          completedJobs.push(job);
        }
      });
      
      console.log(`Found ${completedJobs.length} completed jobs out of ${response.data.jobs.length} total jobs`);
      
      if (completedJobs.length > 0) {
        await fs.writeFile('output/completed-jobs-curl.json', JSON.stringify(completedJobs, null, 2));
        console.log('Saved completed jobs to output/completed-jobs-curl.json');
        console.log('Sample completed job IDs:');
        completedJobs.slice(0, 5).forEach(job => {
          console.log(`- ID: ${job.id}, Internal ID: ${job.internal_id}, Status: ${job.status_name}`);
        });
      }
    }
    ...
    console.error('Error fetching status board data:', error.message);
    ...
    if (error.response) {
      ...
      try {
        await fs.mkdir('output', { recursive: true });
        await fs.writeFile('output/status-board-data-curl-error.json', JSON.stringify(error.response.data, null, 2));
        console.log('Saved error response to output/status-board-data-curl-error.json');
      } catch (writeError) {
...
// Execute the request
fetchStatusBoardData().catch(console.error); 