/**
 * Test script to fetch a specific job by ID
 * This will help us verify we can access completed job data
 */
...
/**
 * Fetch a specific job by ID using the job_card/load_from_job endpoint
 */
async function fetchJobById(jobId) {
  try {
    ...
    await fs.writeFile(`output/job-${jobId}.json`, JSON.stringify(response.data, null, 2));
    console.log(`Saved job data to output/job-${jobId}.json`);
    
    // Print some basic job info
    if (response.data && response.data.value && response.data.value.job) {
      const job = response.data.value.job;
      console.log(`
Job details:
  ID: ${job.id}
  Internal ID: ${job.internal_id || 'N/A'}
  Description: ${job.brief_description || job.description || 'N/A'}
  Status: ${job.job_status || job.status_name || 'N/A'}
  Customer: ${job.customer_full_name || 'N/A'}
  Site Address: ${typeof job.site_address === 'string' ? job.site_address : 'Complex Address Object'}
`);
    }
    ...
    if (error.response) {
      ...
      try {
        await fs.mkdir('output', { recursive: true });
        await fs.writeFile(`output/job-${jobId}-error.json`, JSON.stringify(error.response.data, null, 2));
        console.log(`Saved error response to output/job-${jobId}-error.json`);
      } catch (writeError) {
...
// Choose a sample job ID from the status_board/data response
// Let's try one of the IDs we saw in the job_ids arrays
const sampleJobId = "12238218"; // Change this to test different job IDs

// Fetch the job
fetchJobById(sampleJobId).catch(console.error); 