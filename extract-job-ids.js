/**
 * Script to extract all job IDs from the status_board/data endpoint
 * This will help us get a comprehensive list of jobs in all statuses
 */
...
/**
 * Recursively extract all job_ids from an object
 */
function extractJobIds(obj, jobIds = new Set()) {
  if (!obj || typeof obj !== 'object') return jobIds;
  
  // If we found a job_ids array, add all IDs to our set
...
/**
 * Fetch data from status_board/data and extract all job IDs
 */
async function extractAllJobIds() {
  try {
    console.log('Fetching status board data...');
    ...
    const jobIds = extractJobIds(response.data);
    
    console.log(`Found ${jobIds.size} unique job IDs`);
    ...
    await fs.writeFile('output/all-job-ids.json', JSON.stringify(jobIdsArray, null, 2));
    console.log('Saved job IDs to output/all-job-ids.json');
    
    // Print sample of job IDs
    console.log('Sample job IDs:');
    jobIdsArray.slice(0, 10).forEach(id => {
      console.log(`- ${id}`);
    });
...
// Run the extraction
extractAllJobIds().catch(console.error); 